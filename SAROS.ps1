...
# ============================================================================
# MUTABLE STATE — all script-scope variables declared here
# ============================================================================
...
function Get-VersionLabel ([int]$BuildNumber) {
    $match = $Script:BUILD_TO_VERSION.GetEnumerator() |
        Where-Object { $BuildNumber -ge $_.Key } |
...
# ============================================================================
# FUNCTIONS — Microsoft Update Catalog
# ============================================================================
...
function Invoke-PatchFetch ([string]$SourcePath, [string]$WinVer, [int]$BuildNumber) {
    $verLabel = Get-VersionLabel $BuildNumber
    if (-not $verLabel) { Write-Log "Unknown build $BuildNumber — skipping patch check" -Level Warn; return }
    ...
    $updDir = Join-Path $SourcePath 'Updates'; New-Item $updDir -ItemType Directory -Force -EA SilentlyContinue | Out-Null
    $downloaded = 0
...
function Start-SAROSBuild {
    ...
    # STEP 3 — Mount ISO
    # ================================================================
    Write-Step 3 $totalSteps 'Mounting ISO'
    $isoMount = Mount-SourceISO $isoFile
    ...
    try {
        ...
        Invoke-PatchFetch $srcPath $winVer $buildNum
    } catch {
    ...
    if (Test-Path "$ssuPath\*") { Write-Log 'Injecting SSU';      Add-WindowsPackage -PackagePath $ssuPath -Path $dismDir | Out-Null }
    if (Test-Path "$updPath\*") { Write-Log 'Injecting updates';  Add-WindowsPackage -PackagePath $updPath -Path $dismDir | Out-Null }
