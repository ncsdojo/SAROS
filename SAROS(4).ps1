...
function Invoke-DismWithProgress ([string]$Label, [string[]]$Arguments) {
    $barWidth = 30
    $lastPct  = -1
    $phase    = $Label
    ...
    & dism.exe @Arguments 2>&1 | ForEach-Object {
        ...
        # Detect phase from DISM output text
        if     ($line -match 'scanning|inventory')   { $phase = 'Scanning' }
        elseif ($line -match 'clean|reset|removing')  { $phase = 'Cleaning' }
        elseif ($line -match 'saving|committing')     { $phase = 'Saving' }
        elseif ($line -match 'compress|export|captur') { $phase = 'Compressing' }

        # Also detect phase from percentage ranges
        if ($line -match '(\d+\.?\d*)%') {
            $pct = [int][math]::Round([double]$Matches[1])
            if ($pct -ne $lastPct) {
                # Phase from percentage if no text-based detection
                if ($phase -eq $Label) {
                    if    ($pct -lt 20) { $phase = 'Scanning' }
                    elseif ($pct -lt 70) { $phase = 'Processing' }
                    else                { $phase = 'Finalising' }
                }
                $padPhase = $phase.PadRight(14)
                $filled   = [math]::Round($pct / 100 * $barWidth)
                $empty    = $barWidth - $filled
                $bar      = "$([string]::new([char]0x2588, $filled))$([string]::new([char]0x2591, $empty))"
                [Console]::Write("`r  $padPhase $bar ${pct}%   ")
                $lastPct = $pct
            }
        }
    }
    $done = 'Done'.PadRight(14)
    [Console]::WriteLine("`r  $done $([string]::new([char]0x2588, $barWidth)) OK    ")
...
function Invoke-Cleanup {
    ...
    if ($Script:WIMMounted) {
        # Try PowerShell dismount first, fall back to dism.exe
        try { Dismount-WindowsImage -Path (Join-Path (Get-Location) 'DISM-Offline') -Discard -EA SilentlyContinue | Out-Null } catch { }
        try { & dism.exe /Cleanup-Wim 2>$null | Out-Null } catch { }
        $Script:WIMMounted = $false
    }
...
function New-BootableISO ([string]$SourceDir, [string]$OutputPath) {
    ...
    if (-not $oscdimg) {
        $cmd = Get-Command oscdimg.exe -EA SilentlyContinue
...
function Start-SAROSBuild {
    ...
    # Release any orphaned WIM mounts from previous crashed runs
    $orphans = & dism.exe /Get-MountedWimInfo 2>$null | Select-String 'Mount Dir'
    if ($orphans) {
        Write-Log "Releasing $($orphans.Count) orphaned WIM mount(s)"
        & dism.exe /Cleanup-Wim 2>$null | Out-Null
    }
    ...
    if ($freeGB -lt 15) {
        throw "Insufficient disk space on ${buildDrive}. Need 15 GB, have $freeGB GB. Free up space and try again."
    }
    ...
    Write-Host "    Disk     : $freeGB GB free on $buildDrive" -ForegroundColor Gray
    Write-Host ''
    if ($interactive) {
    ...
    Invoke-DismWithProgress 'Component cleanup' @("/Image:$dismDir", '/Cleanup-Image', '/StartComponentCleanup', '/ResetBase')
...
try {
    ...
    $stamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
    Add-Content -Value "$stamp  [ERROR]  $_" -Path $Script:LogFile -EA SilentlyContinue
    Add-Content -Value "$stamp  [ERROR]  $($_.ScriptStackTrace)" -Path $Script:LogFile -EA SilentlyContinue

    # Show one clean message to console
    Write-Host "`n  FATAL: $_" -ForegroundColor Red
    Write-Host "  See log: $Script:LogFile`n" -ForegroundColor DarkGray
